...
class Renderer_Datetime_Picker extends \Fieldset_Field
{
    ...
    public static function renderer($renderer = array())
    {
        ...
        $attributes['data-datetimepicker-options'] = htmlspecialchars(\Format::forge()->to_json($renderer_options['datetimepicker']));
        $attributes['data-datetimepicker-type'] = htmlspecialchars(\Format::forge()->to_json($renderer_options['type']));

        return '<input '.array_to_attr($attributes).' />'.static::nextToField($attributes['id']).static::js_init($attributes['id'], $renderer_options);
    }
    ...
    public function build()
    {
        ...
        $datepicker_options = $this->options['datetimepicker'];
        $this->set_attribute('data-datetimepicker-options', htmlspecialchars(\Format::forge()->to_json($datepicker_options)));
        $this->set_attribute('data-datetimepicker-type', htmlspecialchars($this->options['type']));

        return ((string) parent::build()).static::nextToField($this->get_attribute('id'));
    }

    public static function nextToField($id) {
        return '<input type="text" id="'.$id.'_hidden" />';
    }
    ...
    {
        $renderer['type'] = 'text';
        $renderer['class'] = (isset($renderer['class']) ? $renderer['class'] : '').' datetimepicker';
    ...
    {
        return \View::forge('renderer/date_time_picker', array(
            'id' => $id,
